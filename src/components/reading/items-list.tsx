  ...
  return (
    <Grid item container sx={{flexDirection: {xs: "row", sm: index %2 === 0? "row-reverse" :"row"}, justifyContent: "space-between"}}>
      <Grid item xs={12} sm={5} sx={{alignItems: "center", mb: {xs: 2, sm:0}}}>
        ...
        <Typography variant="body2" gutterBottom sx={{ml: 2, fontWeight: "bold"}}>{item.author}</Typography>
        <br />
        <Typography variant="body1" gutterBottom sx={{ml: 2, fontWeight: "bold"}} color="GrayText">Genre: {item.genre}</Typography>
        <br />
        <Typography variant="body1" gutterBottom sx={{whiteSpace: "pre-line", ml:2}}>{item.review}</Typography>
        <br /><br />
        <Typography variant="body1" sx={{fontWeight: "bold", ml:2}} gutterBottom>Rating: {item.rating} / 10</Typography>
      </Grid>
